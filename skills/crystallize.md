# Skill: Knowledge Crystallization (結晶化)

## Purpose
NFD（Nurture-First Development）論文に基づき、全環境から蓄積された経験ログをGit経由で収集し、パターンを抽出してスキル参照ファイルを更新する。
...
- 前回の結晶化レポート（crystallization-report-*.md）の日付以降に追加されたエントリを特定
- タグ別に分類: [INSIGHT], [PATTERN], [ERROR], [DECISION], [REASONING], [CONTEXT], [FRAMEWORK], [ROUTING], [PREFERENCE]
- 環境タグ（env:local, env:raspi, env:gpu-lab）も集計し、どの環境からの学びかを識別
...
- 特定の日時・PR番号等の状況固有情報を除去し、再利用可能な一般原則に変換

### Step 4: スキル参照ファイルの更新案を作成
skills/ 配下の各ファイルについて:
- 「Learned Patterns」セクションへの追記案
- 「Error Cases」セクションへのエラーパターン追記案
- 新しいスキルファイルが必要な場合は作成案
- **更新案はドラフトとしてユーザーに提示し、承認後に反映する**

### Step 5: Constitutional層の見直し
- constitutional/CLAUDE.md の内容が現在の実践と一致しているか確認
- constitutional/profile.md に対話から得た新情報を追加すべきか
- Phase昇格やSkill Routing変更がある場合は config/user-level-CLAUDE.md も更新
...
### Step 7: ユーザー承認後、Git commit & push
```bash
...
## 注意事項
- 経験ログが前回以降ない場合は「まだ結晶化するデータがありません」と報告して終了
